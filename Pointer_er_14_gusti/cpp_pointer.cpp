...
void solve(lli tc){
    int *n; // variable. but it stores only the address of the variable
    int x = 7; //normal integer variable
    print(&x) //print the address where x is stored
    n = &x; //&x means address of x;
    n++; // it will add 4 bit with the address of n;
    n--; // it will decrease 4 bit with the address of n;
    print(n) //print the address of n 
    print(*n) //print the integer value that stored in address of n
}
